...
if (!empty($data->Patient_num) && !empty($data->Date) && !empty($data->New_date)){
	$visit -> patient = $data -> Patient_num;
	$visit -> date = $data -> Date;
	$visit -> new_date = $data ->New_date;
	if ($visit-> change()){
		http_response_code(200);
		echo json_encode(array("message" => "appointment successfully changed"));
	}
	else{
		http_response_code(503);
		echo json_encode(array("message" => "appointment not changed"));
	}
}
else{
